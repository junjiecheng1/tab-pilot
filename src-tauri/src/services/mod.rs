...
impl AppServices {
    ...
    pub async fn handle_request(&self, method: &str, params: Value) -> ServiceResult {
        let path = method.strip_prefix("app/").unwrap_or(method);

        let (service, action) = path.split_once('.').ok_or_else(|| {
            ServiceError::bad_request(format!("格式错误: {method} (需要 app/<service>.<action>)"))
        })?;
    ...
    async fn guarded_shell(&self, action: &str, params: Value) -> ServiceResult {
        ...
        if action == "exec" {
            ...
            match decision {
                GuardDecision::Deny => {
                    // 审计: 记录被拒绝
                    self.audit
                        .log("shell", action, &params, "denied by guard", -1, 0.0, "deny")
                        .await;
        ...
        let log_id = self
            .audit
            .log_start("shell", action, &params, "allow")
            .await;
        ...
        if let Some(id) = log_id {
            ...
            self.audit
                .log_finish(id, &preview, exit_code, duration, status)
                .await;
        }
    ...
    async fn guarded_file(&self, action: &str, params: Value) -> ServiceResult {
        ...
        let is_write = matches!(
            action,
            "write" | "append" | "delete" | "move" | "rename" | "mkdir"
        );
        if is_write {
            let decision = {
                let guard = self.guard.read().await;
            ...
            match decision {
                GuardDecision::Deny => {
                    self.audit
                        .log("file", action, &params, "denied by guard", -1, 0.0, "deny")
                        .await;
    ...
    async fn audited_browser(&self, action: &str, params: Value) -> ServiceResult {
        ...
        let log_id = self
            .audit
            .log_start("browser", action, &params, "allow")
            .await;
        ...
        match action {
            ...
            "get_server" => {
                ...
                match self.mcp_client.get_server(name) {
                    Some(cfg) => {
                        serde_json::to_value(cfg).map_err(|e| ServiceError::internal(e.to_string()))
                    }
                    None => Err(ServiceError::not_found(format!("服务器不存在: {name}"))),
                }
            }
            _ => Err(ServiceError::bad_request(format!(
                "未知 mcp_client 操作: {action}"
            ))),
        }
    }
