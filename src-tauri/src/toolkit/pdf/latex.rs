...
pub fn latex_to_pdf(tex_content: &str, output: &Path) -> Result<Value, TabClientError> {
...
fn compile_with_cli(
    ...
    std::fs::create_dir_all(&tmp_dir).map_err(|e| TabClientError::Other(format!("{e}")))?;

    let tex_path = tmp_dir.join("input.tex");
    std::fs::write(&tex_path, tex_content).map_err(|e| TabClientError::Other(format!("{e}")))?;
    ...
    if pdf_path.exists() {
        std::fs::copy(&pdf_path, output).map_err(|e| TabClientError::Other(format!("{e}")))?;
...
/// LaTeX 文件 → PDF
pub fn latex_file_to_pdf(input: &Path, output: &Path) -> Result<Value, TabClientError> {
