...
impl BrowserManager {
    ...
    async fn wait_for_lifecycle(
        ...
        self.wait_for_lifecycle_with_timeout(wait_until, session_id, rx, timeout)
            .await
    }

    async fn wait_for_lifecycle_with_timeout(
        &self,
        wait_until: WaitUntil,
        session_id: &str,
        rx: &mut broadcast::Receiver<CdpEvent>,
        ...
        let event_name = match wait_until {
            ...
            WaitUntil::NetworkIdle => return poll_network_idle(session_id, rx, timeout).await,
        };
    ...
    pub async fn wait_for_lifecycle_external_with_timeout(
        &self,
        wait_until: WaitUntil,
        session_id: &str,
        timeout: Duration,
        ...
        let mut rx = self.client.subscribe();
        self.wait_for_lifecycle_with_timeout(wait_until, session_id, &mut rx, timeout)
            .await
    }
