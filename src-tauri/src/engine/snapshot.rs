...
fn render_tree(
    ...
    // 跳过纯文本布局节点 — Chrome ARIA 树会返回 StaticText / InlineTextBox,
    // 但它们的文本内容已包含在父节点的 name 中, 渲染出来只是重复噪声。
    // Playwright 的 accessibility.snapshot(interesting_only=true) 也会过滤掉它们。
